...
for case_ind = 1 : length(validate)
	...
	for i = 1 : 30
		...
		range = load(strcat(ROOT_DIR,'IC-Range\Range-', int2str(i), '.txt'));
	...
	avgAllError = mean(allErrors) / 2;
	avgAbsAllError = mean(abs(allErrors)) / 2;
	stdAllError = std(allErrors) / 2;
	stdAbsAllError = std(abs(allErrors)) / 2;