...
for case_ind = 1 : length(validate)
	...
	ROOT_DIR = strcat('J:\Roliroli\S', number,'\TO-Time-Validated\');
	OUT_DIR = strcat('J:\Roliroli\S', number,'\TO-Time\');
	TEST_CASE_FILE = strcat('J:\Roliroli\S', number,'\autoSVMBuild.xlsx');
	OUT_FILE_NAME = strcat('J:\Roliroli\S', number, '\Correlation-TO.txt');
