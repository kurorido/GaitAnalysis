...
for case_ind = 1 : length(validate)
	...
	OUT_DIR = strcat(ROOT_DIR, 'CoM-Speed\Combination-', int2str(Combination), '\');
	mkdir(OUT_DIR);
	...
	dlmwrite(strcat(OUT_DIR, 'CoMSpeed-Self.txt'), OUT_Self);
	dlmwrite(strcat(OUT_DIR, 'CoMSpeed-100.txt'), OUT_100);
	dlmwrite(strcat(OUT_DIR, 'CoMSpeed-120.txt'), OUT_120);
	dlmwrite(strcat(OUT_DIR, 'CoMSpeed-All.txt'), [OUT_Self ; OUT_100 ; OUT_120]);
